...
def cluster_analysis(positions, cutoff):
    """
    Perform cluster analysis using a cutoff distance.
    """
...
def save_clusters(universe, clusters, unwrapped_positions, frame_num):
    ...
    for idx, cluster in enumerate(clusters):
        ...
        cluster_filename = f"{output_dir}/frame{frame_num}_cluster{idx+1}_size{cluster_size}.gro"
...
def main():
    ...
    box_size = u.dimensions[:3]  # Extract box dimensions

    for ts in u.trajectory[args.first:args.last:args.skip]:
        positions = u.select_atoms('all').positions
        images = np.zeros_like(positions)  # Assuming no image flags available; modify if needed

        # Unwrap particle coordinates
        unwrapped_positions = unwrap_coordinates(positions, box_size, images)

        # Perform cluster analysis
        cluster_sizes, clusters = cluster_analysis(unwrapped_positions, args.cutoff)
