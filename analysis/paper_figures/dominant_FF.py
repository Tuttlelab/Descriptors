...
def load_all_descriptors():
    ...
    adi = pd.read_csv('FF/adi_output.csv')
    sfi = pd.read_csv('FF/sfi_output.csv')
    ffi = pd.read_csv('FF/ffi_output.csv')
    tfi = pd.read_csv('FF/tfi_output.csv')
    vfi = pd.read_csv('FF/vfi_output.csv')
...
def plot_aggregation_analysis(df, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'FF/dominant_{timestamp}.png', dpi=600, bbox_inches='tight')
...
def main():
    ...
    df[['Frame', 'shapes']].to_csv(
        f'FF/dominant_{timestamp}.csv', index=False)
